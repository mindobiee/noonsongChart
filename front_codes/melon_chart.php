...
echo '<h2 class ="clock" style="color : black">'.date("Y.m.d", time()).'
    <span style = "color: #01cc3b">'.date("H:00", time()).'
    </span></h2>';
echo'<h5 style="text-align: center">※ 순위 비교는 기존 사이트 대비 chartrue의 순위와의 차이를 보여줍니다.</h5>';
    //※ 순위 비교는 기존의 음원사이트 대비 차투르 차트 랭킹 증가순위를 의미합니다.
echo'
    <section>
...
$query = 'select * from musicList_melon order by ranking asc';
...
while($row = mysqli_fetch_array($result)){
    ...
    echo'<td class="comment_link" style="color:#01cc3b"><strong>'.$row['ranking'].'</strong></td>';
    if($cha>0 && $cha <101)
    ...
    <a  href="comment.php?page=melon&title='.$row['title'].'">
    ...
    echo'<td title="해당 음원사이트의 곡페이지로 넘어갑니다."><a href ='.$row['song_url'].'><img src = "melonb.jpg" width = "30px" height="30px"></a></td>';
    echo '</tr>';
    $i+=1;
}
mysqli_close($conn);
