...
echo '<h2 class="clock" style="color : black">'.date("Y.m.d", time()).'
    <span style = "color: #11a6e3">'.date("H:00", time()).'
    </span></h2>';
echo'<h5 style="text-align: center;">※ 순위 비교는 기존 사이트 대비 chartrue의 순위를 보여줍니다.</h5>';
echo'
    <section>
...
$query = 'select * from musicList_genie order by ranking asc';
...
while($row = mysqli_fetch_array($result)){
    ...
    elseif($cha<-20)
        ...
        echo'<tr>';
    echo'<td style="color: #11a6e3"><strong>'.$row['ranking'].'</strong>
        </a></td>';
    if($cha>0 && $cha <101)
    ...
    <a  href="comment.php?page=genie&title='.$row['title'].'">
    ...
    echo'<td title="해당 음원사이트의 곡페이지로 넘어갑니다."><a href ='.$row['song_url'].'><img src = "genieb.jpg" width = "30px" height="30px"></a></td>';
    echo '</tr>';
    $i+=1;
}
mysqli_close($conn);
